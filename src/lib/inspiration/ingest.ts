  ...
  const response = await new Promise<{
    ...
    const req = (url.protocol === "https:" ? httpsRequest : httpRequest)(
      url,
      {
        ...
        headers: {
          "User-Agent": "BLANK-Inspiration/1.0",
      ...
      (res) => {
        ...
        res.on("data", (chunk: Buffer) => {
          bytes += chunk.length;
          if (bytes > 2_000_000) {
            res.destroy(new Error("Source exceeds 2 MB."));
            return;
          }
          chunks.push(chunk);
        });
...
export async function extractSource(
  ...
  if (!/html/i.test(contentType))
    throw new Error("Only HTML and Markdown sources are supported.");
  ...
  try {
    ...
    if (!article?.content)
      throw new Error("No readable source body. Manual review is required.");
    const extracted = new JSDOM(article.content);
    try {
      return [
        ...extracted.window.document.querySelectorAll(
          "h1,h2,h3,h4,h5,h6,p,pre,li,table",
        ),
      ]
        .filter((node) => !node.parentElement?.closest("pre,li,table"))
        .map(
          (node) =>
            `${/^H[1-6]$/.test(node.tagName) ? `${"#".repeat(Number(node.tagName[1]))} ` : ""}${node.textContent?.trim() ?? ""}`,
        )
        .filter(Boolean)
        .join("\n\n");
    } finally {
...
export function chunkSource(
  ...
  const flush = () => {
    ...
    passages.push({
      id: `${resourceId}_${hash.slice(0, 16)}_${ordinal}`,
      resourceId,
      sourceUrl,
...
/** Recheck one active source. Dead sources quarantine; moved or changed ones
 *  report for a curator. Never deletes history and never rewrites the href:
 *  a moved URL is a curation decision, not an automatic update. */
export async function checkFreshness(
  db: InspirationDatabase,
  resource: { id: string; href: string },
) {
  let raw: { url: string; body: string; contentType: string };
  try {
    raw = await fetchSource(resource.href);
  } catch (error) {
    return {
      id: resource.id,
  ...
  if (canonicalUrl(raw.url) !== canonicalUrl(resource.href)) {
    return { id: resource.id, status: "moved" as const, detail: raw.url };
  }
  let extracted: string;
  try {
    extracted = await extractSource(raw.body, raw.contentType, raw.url);
  } catch {
    return {
      id: resource.id,
      status: "unreadable" as const,
      detail: "Source responds but yields no readable text.",
    };
  }
  const hash = createHash("sha256")
    .update(JSON.stringify([raw.url, extracted]))
    .digest("hex");
  const [latest] = await db.query<{ hash: string }>(
    "SELECT hash FROM inspiration_snapshots WHERE resource_id = $1 ORDER BY fetched_at DESC LIMIT 1",
    [resource.id],
  );
  if (!latest)
    return {
      id: resource.id,
      status: "ok" as const,
      detail: "First sighting. No snapshot to compare.",
    };
  if (latest.hash !== hash)
    return {
      id: resource.id,
      status: "changed" as const,
      detail: "Content hash differs from the last snapshot.",
    };
  return {
    id: resource.id,
    status: "ok" as const,
    detail: "Matches the last snapshot.",
  };
}

export async function enqueue(db: InspirationDatabase, resourceId: string) {
  if (!(await resolveResource(db, resourceId)))
...
export async function publishSource(
  db: InspirationDatabase,
  job: { resource_id: string; lease_token: string },
  passages: Passage[],
  locator: string,
) {
  if (!passages.length)
    throw new Error("Empty source cannot replace the last successful version.");
  ...
  if (!rows.length)
    throw new Error(
      "Source lease expired. Another worker may have claimed it.",
    );
}

/** CLI-invoked drain may target any database. Server routes must never call
 *  this: one invocation fetches up to `limit` sources with no request-scoped
 *  budget. The CLI sets INSPIRATION_INGEST_CLI; routes never do. */
export async function drainJobs(db: InspirationDatabase, limit = 6) {
  if (process.env.INSPIRATION_INGEST_CLI !== "1")
    throw new Error("Source ingestion requires an explicit CLI invocation.");
  const results: {
    resourceId: string;
    state: string;
    passages?: number;
  ...
  for (
    let n = 0;
    n < Math.min(limit, 6) && Date.now() < deadline - 12000;
    n++
  ) {
    const job = await claimJob(db);
    if (!job) break;
    // Polite sequential fetching: one source at a time with a pause between.
    if (n > 0) await new Promise((resolve) => setTimeout(resolve, 1500));
    try {
      const resource = await resolveResource(db, job.resource_id);
      if (!resource) throw new Error("Resource is inactive.");
      ...
      if (extracted.length < 200)
        throw new Error(
          "Too little readable text. Manual source inspection is required.",
        );
      const hash = createHash("sha256")
        .update(JSON.stringify([raw.url, extracted]))
        .digest("hex");
      const passages = chunkSource(
        extracted,
        resource.id,
        raw.url,
        hash,
      ...
      const directory = resolve(
        process.env.INSPIRATION_LOCAL_DB || ".inspiration-local",
        "snapshots",
      );
