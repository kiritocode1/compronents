...
class DependencyDown extends Data.TaggedError("DependencyDown")<{
  readonly why: string;
}> {}
class CircuitOpen extends Data.TaggedError("CircuitOpen")<{
  readonly retryAfterMillis: number;
}> {}
class BulkheadFull extends Data.TaggedError("BulkheadFull")<{
  readonly dependency: string;
}> {}
class RetryBudgetExhausted extends Data.TaggedError(
  ...
  Effect.map(
    ...
    (tokens) =>
      ({
        tokens,
        cap: options.cap,
        earnPerCall: options.earnPerCall,
      }) satisfies RetryBudget,
  );
...
const withBudgetedRetry =
  ...
  <A, E, R>(
  ...
  ): Effect.Effect<A, E | RetryBudgetExhausted, R> => {
    ...
    const spend: Effect.Effect<A, E | RetryBudgetExhausted, R> = Effect.catch(
      effect,
      (err: E) =>
        Effect.flatMap(
          Ref.modify(budget.tokens, (t) =>
            t >= 1 ? ([true, t - 1] as const) : ([false, t] as const),
          ),
          (hasToken): Effect.Effect<never, E | RetryBudgetExhausted> =>
            hasToken
              ? Effect.fail(err)
              : Effect.fail(new RetryBudgetExhausted()),
        ),
    );
    return Effect.andThen(
      Ref.update(budget.tokens, (t) =>
        Math.min(budget.cap, t + budget.earnPerCall),
      ),
      Effect.retry(spend, {
        schedule: policy,
        while: (e) => !(e instanceof RetryBudgetExhausted),
      }),
    );
  };
  ...
  Effect.map(
    ...
    (state) =>
      ({
        ...
        cooldownMillis: Duration.toMillis(options.cooldown),
      }) satisfies Breaker,
  );
    ...
    Effect.gen(function* () {
      ...
      if (typeof gate === "object") {
        return yield* Effect.fail(
          new CircuitOpen({ retryAfterMillis: Math.max(0, gate.reject) }),
        );
      }

      const onSuccess = Ref.set(breaker.state, {
        _tag: "closed",
        failures: 0,
      } as BreakerState);
      const onFailure = Ref.update(breaker.state, (s): BreakerState => {
        if (s._tag === "halfOpen")
          return { _tag: "open", until: now + breaker.cooldownMillis };
...
const makeBulkhead = (dependency: string, permits: number) =>
  Effect.map(Semaphore.make(permits), (sem) => ({
    run: <A, E, R>(
      effect: Effect.Effect<A, E, R>,
    ): Effect.Effect<A, E | BulkheadFull, R> =>
      Semaphore.withPermitsIfAvailable(
        ...
        Effect.flatMap(
          Option.match({
            onNone: () => Effect.fail(new BulkheadFull({ dependency })),
...
const makeDependency = (options: {
  ...
  Effect.gen(function* () {
    ...
    const call = Effect.gen(function* () {
      ...
      if (n <= options.failFirst)
        return yield* Effect.fail(new DependencyDown({ why: `attempt ${n}` }));
      return `ok@${n}`;
...
const demo = Effect.gen(function* () {
  ...
  // Property 1: breaker opens after `threshold` failures and short-circuits.
  {
    ...
    const lastTwo = yield* Effect.all([
      Effect.result(guarded),
      Effect.result(guarded),
    ]);
    const attemptsFinal = yield* Ref.get(dep.attempts);
    const shortCircuited = lastTwo.every(
      (r) => r._tag === "Failure" && r.failure instanceof CircuitOpen,
    );
    yield* assert(
  ...
  // Property 3: retry budget caps total retries at earnPerCall * traffic.
  {
    ...
    const dep = yield* makeDependency({
      failFirst: 100000,
      latency: "1 millis",
    }); // always fails
    const traffic = 50;
    yield* Effect.all(
      Array.from({ length: traffic }, () =>
        Effect.result(withBudgetedRetry(budget)(dep.call)),
      ),
      { concurrency: 10 },
    );
  ...
  // Property 4: bulkhead rejects overflow instead of queueing on a saturated dep.
  {
    ...
    const rejected =
      results._tag === "Failure" && results.failure instanceof BulkheadFull;
    yield* assert(
