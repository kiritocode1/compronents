/**
 * The volume shader for Video Summagator.
 *
 * The box is drawn BackSide so every fragment is a point on the far wall of the
 * cube, which gives the ray a guaranteed exit surface to start from. The camera
 * is orthographic, so rays are parallel: the direction comes from the view
 * matrix's third column rather than from `fragmentPosition - cameraPosition`,
 * and each fragment's origin is projected back onto the camera plane.
 *
 * Time runs along +Z to -Z. Everything from the selected frame onward is opaque,
 * so the march only integrates the slab in front of it and then lands exactly on
 * the boundary plane instead of stepping over it.
 *
 * BLANK - aryank.space
 */
...
/** Ray-march step count. Also the loop bound, so it must stay a constant. */
export const VOLUME_STEPS = 160;
...
vec3 sampleVideo(vec3 p) {
  ...
  // Sample texel centres so the first and last time positions land on the
  // first and last captured frames rather than half a texel outside them.
...
vec3 outlinedFrame(vec3 p) {
  // Measure against the projected edges in CSS pixels, so the stroke keeps its
  // width independent of zoom, viewing angle, DPR and the drawing-buffer cap.
  ...
  // A 1 CSS px inside stroke, antialiased over one drawing-buffer pixel.
  float border = 1. - smoothstep(1. - aa, 1. + aa, distanceToEdge);
  return mix(sampleVideo(p), vec3(1.), border);
}

void main() {
  // Orthographic rays are parallel. Project each fragment back onto the camera
  // plane instead of starting the ray at the camera position.
  ...
  // Time advances toward -Z. The selected frame and everything after it form an
  // opaque sub-volume; intersect its boundary exactly, independent of step size.
  ...
  float stepSize = max(transparentEnd - entry, 0.) / float(${VOLUME_STEPS});
  float alpha = 0.;
