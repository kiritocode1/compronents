...
subscribe("http.server.response.finish", (m) => {
	const { request, response } = m as { request: IncomingMessage; response: ServerResponse };
	...
	log("request", { reqId: ctx.id, route: ctx.route, status: response.statusCode, durMs: Number(durMs.toFixed(2)), outbound: ctx.outbound });
});
...
subscribe("undici:request:create", (m) => {
	const { request } = m as { request: { origin: string; method: string; path: string } };
	outboundTotal++;
	const ctx = als.getStore();
	if (ctx) ctx.outbound++;
	log("outbound", { reqId: ctx?.id ?? null, method: request.method, target: `${request.origin}${request.path}` });
});
...
function renderMetrics(): string {
	...
	for (const [route, h] of latencyByRoute) {
		...
		BUCKETS_MS.forEach((b, i) => {
			cum += h.counts[i];
			lines.push(`http_request_duration_ms_bucket{route="${route}",le="${b}"} ${cum}`);
		});
		lines.push(`http_request_duration_ms_bucket{route="${route}",le="+Inf"} ${h.total}`);
		lines.push(`http_request_duration_ms_sum{route="${route}"} ${h.sum.toFixed(3)}`);
	...
	for (const [key, n] of statusByRoute) {
		...
		lines.push(`http_responses_total{route="${route}",status="${status}"} ${n}`);
	}
	lines.push("# TYPE net_server_sockets_total counter", `net_server_sockets_total ${serverSockets}`);
	lines.push("# TYPE outbound_requests_total counter", `outbound_requests_total ${outboundTotal}`);
	return lines.join("\n") + "\n";
}
...
const server = createServer(async (req, res) => {
	...
	if (path === "/ping") {
		...
		const upstream = await fetch(`http://127.0.0.1:${PORT}/ping`).then((r) => r.json());
