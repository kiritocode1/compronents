...
interface Ring {
  readonly slots: ReadonlyArray<{ readonly position: number; readonly shard: number }>
}

const makeRing = (shardCount: number, vnodesPerShard = 64): Ring => {
  const slots: Array<{ position: number; shard: number }> = []
  for (let shard = 0; shard < shardCount; shard++) {
    for (let v = 0; v < vnodesPerShard; v++) slots.push({ position: hash(`shard:${shard}:vnode:${v}`), shard })
  }
...
const makeShard = (id: number, capacity: number) =>
  Effect.gen(function* () {
    ...
    const gauge = Metric.gauge(`shard_queue_depth_${id}`, { description: `live queue depth for shard ${id}` })
    return { id, queue, depth, gauge, processed, received } satisfies Shard
  })

const makeRouter = (options: { readonly shardCount: number; readonly capacity: number; readonly hotThreshold: number }) =>
  Effect.gen(function* () {
    const ring = makeRing(options.shardCount)
    const shards = yield* Effect.all(
      Array.from({ length: options.shardCount }, (_, i) => makeShard(i, options.capacity)),
    )
    const hotCounts = yield* Ref.make(new Map<string, number>())
    return { ring, shards, hotCounts, hotThreshold: options.hotThreshold } satisfies Router
  })
...
const demo = Effect.gen(function* () {
  ...
  // Property 1: a bounded shard queue applies backpressure when full.
  {
    ...
    for (let i = 0; i < 4; i++) yield* Queue.offer(q, { key: "k", payload: `${i}` })
    // The 5th offer must not complete while the queue is full.
    const blocked = yield* Queue.offer(q, { key: "k", payload: "5" }).pipe(Effect.timeout("60 millis"), Effect.result)
    const wasBlocked = blocked._tag === "Failure"
    // Draining one slot lets the pending offer through.
    yield* Queue.take(q)
    const nowAccepts = yield* Queue.offer(q, { key: "k", payload: "5" }).pipe(Effect.timeout("60 millis"), Effect.result)
    yield* assert(
  ...
  {
    const router = yield* makeRouter({ shardCount: 8, capacity: 10000, hotThreshold: 3 })
  ...
  {
    const router = yield* makeRouter({ shardCount: 4, capacity: 100, hotThreshold: 1000 })
    const gaugeBefore = yield* Effect.scoped(
      Effect.gen(function* () {
        // Enqueue without workers so depth is observable, then read the gauge.
        for (let i = 0; i < 12; i++) yield* submit(router, { key: `key-${i}`, payload: `${i}` })
