...
export interface Replica {
  ...
  readonly get: (key: string) => Effect.Effect<Versioned | undefined, ReplicaDown>
...
export const makeReplica = (name: string): Effect.Effect<Replica> =>
  Effect.gen(function* () {
    ...
    const guard = Effect.gen(function* () {
      if (!(yield* Ref.get(up))) return yield* new ReplicaDown({ replica: name })
    })
    const get = (key: string) =>
      guard.pipe(Effect.andThen(Ref.get(table)), Effect.map((m) => m.get(key)))
...
export interface QuorumStore {
  readonly write: (key: string, value: string) => Effect.Effect<number, QuorumUnreachable>
  readonly read: (
    key: string,
  ) => Effect.Effect<{ value: string; version: number; repaired: readonly string[] }, QuorumUnreachable>
}
  ...
  Effect.gen(function* () {
    if (options.r + options.w <= replicas.length) {
      return yield* Effect.die(
        new Error(`R + W must exceed N: got R=${options.r} W=${options.w} N=${replicas.length}`),
      )
    }
    ...
    const write = (key: string, value: string) =>
      Effect.gen(function* () {
        ...
        const acks = yield* Effect.all(
          replicas.map((rep) => rep.put(key, { value, version }).pipe(Effect.exit)),
          { concurrency: "unbounded" },
        )
        const ok = acks.filter((e) => e._tag === "Success").length
        if (ok < options.w) {
          return yield* new QuorumUnreachable({ needed: options.w, reached: ok })
        }
        return version
      })
    ...
    const read = (key: string) =>
      Effect.gen(function* () {
        const answers = yield* Effect.all(
          replicas.map((rep) =>
            rep.get(key).pipe(Effect.map((v) => ({ rep, v })), Effect.exit),
          ),
          { concurrency: "unbounded" },
        )
        const alive = answers.flatMap((e) => (e._tag === "Success" ? [e.value] : []))
        if (alive.length < options.r) {
          return yield* new QuorumUnreachable({ needed: options.r, reached: alive.length })
        }
        ...
        if (newest.v === undefined) {
          return yield* new QuorumUnreachable({ needed: options.r, reached: 0 })
        }
        const winner = newest.v
        const stale = alive.filter((a) => (a.v?.version ?? -1) < winner.version)
        ...
        return { value: winner.value, version: winner.version, repaired: stale.map((s) => s.rep.name) }
      })
...
const demo = Effect.gen(function* () {
  ...
  const [r1, r2, r3] = yield* Effect.all([makeReplica("r1"), makeReplica("r2"), makeReplica("r3")])
  ...
  // read returns the NEW value even though a replica holds the old one.
  {
    ...
    yield* check(
      ...
      direct?.value === "theme=light" && quorum.value === "theme=dark" && quorum.version === 2,
  ...
  // Property 4: losing a quorum is a typed failure, not a wrong answer.
  {
    ...
    yield* check(
      ...
      exit._tag === "Failure" && String(exit.cause).includes("QuorumUnreachable"),
