...
export type Finding =
  ...
  | {
      readonly kind: "missing_external";
      readonly txId: string;
      readonly amountCents: number;
    }
  | {
      readonly kind: "missing_internal";
      readonly txId: string;
      readonly amountCents: number;
    }
  | {
      readonly kind: "pending_cutoff";
      readonly txId: string;
      readonly amountCents: number;
    };
    ...
    const run = (
      ...
      Effect.gen(function* () {
        ...
        const ours = new Map<string, Normalized>([
          ...prior.entries(),
          ...internal.map((r) => [r.txId, normalizeLedger(r)] as const),
        ]);
        ...
        for (const [txId, mine] of ours) {
          ...
          if (other === undefined) {
            ...
            const nearCutoff =
              opts.dayEndMs - mine.atMs <= options.cutoffToleranceMs;
        ...
        for (const [txId, theirsOnly] of theirs) {
          if (!ours.has(txId)) {
            findings.push({
              ...
              txId,
              amountCents: theirsOnly.amountCents,
            });
          }
        }
...
const demo = Effect.gen(function* () {
    ...
    [
      {
        reference: "tx-100",
        amount: "49.99",
        settledAt: "2026/07/01 10:00:02",
      },
      {
        reference: "tx-101",
        amount: "25.01",
        settledAt: "2026/07/01 11:30:01",
      }, // 1 cent drift
  ...
  const kind = (id: string) =>
    day1.find((f) => "txId" in f && f.txId === id)?.kind;
  yield* check(
  ...
  yield* check(
    ...
    drift?.kind === "amount_mismatch" &&
      drift.externalCents - drift.internalCents === 1,
