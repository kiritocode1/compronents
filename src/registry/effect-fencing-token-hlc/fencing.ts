...
interface FencedResource {
  readonly cell: Ref.Ref<{ readonly highestToken: number; readonly value: string }>
}

const makeFencedResource = (initial: string) =>
  Effect.map(Ref.make({ highestToken: 0, value: initial }), (cell) => ({ cell }) satisfies FencedResource)
...
const fencedWrite = (resource: FencedResource, token: number, value: string) =>
  Ref.modify(
    resource.cell,
    (state): readonly [WriteDecision, { readonly highestToken: number; readonly value: string }] => {
      if (token < state.highestToken) {
        return [{ ok: false, highestAccepted: state.highestToken }, state]
      }
      return [{ ok: true, highestAccepted: token }, { highestToken: token, value }]
    },
  ).pipe(
    Effect.flatMap((decision) =>
      decision.ok
        ? Effect.void
        : Effect.fail(new StaleLeader({ presentedToken: token, highestAccepted: decision.highestAccepted })),
    ),
  )
      ...
      Clock.currentTimeMillis.pipe(
        Effect.flatMap((now) =>
          Ref.modify(state, (last) => {
            ...
            let logical: number
            if (physical === last.physical && physical === remote.physical) logical = Math.max(last.logical, remote.logical) + 1
...
const demo = Effect.gen(function* () {
  ...
  // leader's write is accepted, and the highest accepted token never regresses.
  {
    ...
    const staleAttempt = yield* fencedWrite(resource, tokenA, "written-by-A").pipe(Effect.result)
    // A retries; still stale.
    const staleRetry = yield* fencedWrite(resource, tokenA, "written-by-A-again").pipe(Effect.result)
    const finalState = yield* Ref.get(resource.cell)

    const rejected = staleAttempt._tag === "Failure" && staleAttempt.failure instanceof StaleLeader
    yield* assert(
      "fencing token rejects the stale leader",
      rejected && staleRetry._tag === "Failure" && finalState.value === "written-by-B" && finalState.highestToken === tokenB,
  ...
  // Property 3: HLC ordering survives a five-minute backward wall-clock jump.
  {
    ...
    const strictlyIncreasing = stamps.every((s, i) => i === 0 || hlcCompare(stamps[i - 1], s) < 0)
    const physicalNeverRegressed = stamps.every((s, i) => i === 0 || s.physical >= stamps[i - 1].physical)
    yield* assert(
