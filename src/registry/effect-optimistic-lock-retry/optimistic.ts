...
export const makeVersionedStore = <A>(): Effect.Effect<VersionedStore<A>> =>
  Effect.gen(function* () {
    ...
    const get = (key: string) =>
      Ref.get(table).pipe(Effect.map((m) => m.get(key)));
    ...
    const compareAndSet = (key: string, expected: number, value: A) =>
      Effect.gen(function* () {
        type Outcome =
          | { readonly _tag: "Ok"; readonly row: Row<A> }
    ...
    const attempt = Effect.gen(function* () {
      ...
      if (row === undefined) {
        return yield* Effect.die(
          new Error(`optimisticUpdate on missing key ${key}`),
        );
      }
      if (options.readWriteGapMs)
        yield* Effect.sleep(`${options.readWriteGapMs} millis`);
  ...
  // Property 4: retry exhaustion on a genuine hot spot is a visible failure.
  {
    ...
    const antagonist = Effect.gen(function* () {
      for (let i = 0; i < 200; i++) {
        ...
        if (row)
          yield* store
            .compareAndSet("hot", row.version, row.value)
            .pipe(Effect.exit);
    ...
    const sawExhaustion =
      exit._tag === "Success" ||
      String(exit.cause).includes("RetriesExhausted");
    yield* check(
