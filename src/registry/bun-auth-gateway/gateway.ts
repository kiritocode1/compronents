...
const db = new Database(process.env.GATEWAY_DB ?? `${import.meta.dir}/.gateway.db`, { create: true });
db.run("PRAGMA journal_mode=WAL");
db.run(`CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, email TEXT UNIQUE NOT NULL, pw_hash TEXT NOT NULL)`);
db.run(`CREATE TABLE IF NOT EXISTS sessions (sid TEXT PRIMARY KEY, user_id INTEGER NOT NULL, expires_at REAL NOT NULL)`);
db.run(`CREATE TABLE IF NOT EXISTS notes (id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL, text TEXT NOT NULL)`);
...
db.run(`CREATE TABLE IF NOT EXISTS request_log (ts REAL, method TEXT, path TEXT, status INTEGER, ms REAL, ip TEXT)`);
...
function rateLimit(key: string, limit: number, windowSec: number): boolean {
  db.run("DELETE FROM hits WHERE key = ? AND ts < unixepoch('subsec') - ?", [key, windowSec]);
  const { c } = db.query("SELECT count(*) c FROM hits WHERE key = ?").get(key) as { c: number };
  if (c >= limit) return false;
  db.run("INSERT INTO hits (key, ts) VALUES (?, unixepoch('subsec'))", [key]);
  return true;
}

function sessionUser(req: Bun.BunRequest): { userId: number; sid: string } | null {
  const sid = req.cookies.get("sid");
  if (!sid) return null;
  const row = db
    .query("SELECT user_id FROM sessions WHERE sid = ? AND expires_at > unixepoch('subsec')")
...
type Handler = (req: Bun.BunRequest, ctx: { userId: number; sid: string }) => Response | Promise<Response>;

/** Wrap a handler with rate limiting, request logging, and optional session + CSRF checks. */
function guard(opts: { auth?: boolean; csrf?: boolean; limit?: number; windowSec?: number }, handler: Handler) {
  return async (req: Bun.BunRequest, server: Bun.Server) => {
    ...
    try {
      if (!rateLimit(`${ip}:${path}`, opts.limit ?? 30, opts.windowSec ?? 10)) {
        res = Response.json({ error: "rate limit exceeded, slow down" }, { status: 429, headers: { "retry-after": String(opts.windowSec ?? 10) } });
      } else {
        ...
        if (opts.auth && !ctx) {
          ...
          res = Response.json({ error: "missing or invalid CSRF token, fetch a fresh one from GET /csrf" }, { status: 403 });
        } else {
    ...
    db.run("INSERT INTO request_log VALUES (unixepoch('subsec'), ?, ?, ?, ?, ?)", [req.method, path, res.status, ms, ip]);
    console.log(`[gateway] ${req.method} ${path} ${res.status} ${ms.toFixed(1)}ms ${ip}`);
    return res;
  };
}
  ...
  return Bun.serve({
    ...
    routes: {
      "/auth/signup": {
        POST: guard({ limit: 5 }, async (req) => {
          ...
          if (!email?.includes("@") || !password || password.length < 8)
            return Response.json({ error: "need a valid email and a password of 8+ characters" }, { status: 400 });
          const pw_hash = await Bun.password.hash(password);
          try {
            const { id } = db.query("INSERT INTO users (email, pw_hash) VALUES (?, ?) RETURNING id").get(email, pw_hash) as any;
      ...
      "/auth/login": {
        POST: guard({ limit: 5 }, async (req) => {
          const { email, password } = (await req.json().catch(() => ({}))) as any;
          const user = db.query("SELECT id, pw_hash FROM users WHERE email = ?").get(email ?? "") as any;
          // verify against a dummy hash when the user is unknown so response timing does not leak account existence
          const ok = await Bun.password.verify(password ?? "", user?.pw_hash ?? DUMMY_HASH);
          if (!user || !ok) return Response.json({ error: "wrong email or password" }, { status: 401 });
          const sid = Bun.randomUUIDv7();
          db.run("INSERT INTO sessions (sid, user_id, expires_at) VALUES (?, ?, unixepoch('subsec') + ?)", [sid, user.id, SESSION_TTL_SEC]);
          req.cookies.set("sid", sid, { httpOnly: true, sameSite: "lax", maxAge: SESSION_TTL_SEC, path: "/" });
      ...
      "/me": {
        GET: guard({ auth: true }, (_req, { userId }) => {
          const user = db.query("SELECT id, email FROM users WHERE id = ?").get(userId) as any;
          return Response.json(user);
        }),
      },
      "/csrf": {
        GET: guard({ auth: true }, (_req, { sid }) =>
          Response.json({ token: Bun.CSRF.generate(csrfSecret(sid), { expiresIn: 60 * 60 * 1000 }) })),
      },
      "/api/notes": {
        GET: guard({ auth: true }, (_req, { userId }) =>
          Response.json(db.query("SELECT id, text FROM notes WHERE user_id = ?").all(userId))),
        POST: guard({ auth: true, csrf: true }, async (req, { userId }) => {
          const { text } = (await req.json().catch(() => ({}))) as any;
          if (!text?.trim()) return Response.json({ error: "note text is required" }, { status: 400 });
          const row = db.query("INSERT INTO notes (user_id, text) VALUES (?, ?) RETURNING id, text").get(userId, text) as any;
          return Response.json(row, { status: 201 });
        }),
      },
      "/admin/logs": {
        GET: guard({}, () =>
          Response.json(db.query("SELECT * FROM request_log ORDER BY ts DESC LIMIT 20").all())),
      },
      "/*": guard({}, () => Response.json({ error: "no such route" }, { status: 404 })),
    },
  });
}
...
if (import.meta.main) {
  ...
  if (arg === "self-test") {
    db.run("DELETE FROM users"); db.run("DELETE FROM sessions"); db.run("DELETE FROM notes");
    db.run("DELETE FROM hits"); db.run("DELETE FROM request_log");
    const server = startGateway(4011);
    const base = "http://localhost:4011";
    const post = (path: string, body: unknown, headers: Record<string, string> = {}) =>
      fetch(base + path, { method: "POST", body: JSON.stringify(body), headers: { "content-type": "application/json", ...headers } });
    const assert = (cond: unknown, msg: string) => { if (!cond) throw new Error(`self-test failed: ${msg}`); };

    let r = await post("/auth/signup", { email: "ada@example.com", password: "correct-horse" });
    assert(r.status === 201, `signup -> ${r.status}`);
    r = await post("/auth/login", { email: "ada@example.com", password: "wrong-horse" });
    assert(r.status === 401, `bad login -> ${r.status}`);
    r = await post("/auth/login", { email: "ada@example.com", password: "correct-horse" });
    ...
    assert(r.status === 200 && (await r.json()).email === "ada@example.com", "/me with session");

    r = await post("/api/notes", { text: "csrf should block this" }, { cookie });
    assert(r.status === 403, `mutation without CSRF -> ${r.status}`);
    const { token } = await (await fetch(base + "/csrf", { headers: { cookie } })).json();
    r = await post("/api/notes", { text: "ship the gateway" }, { cookie, "x-csrf-token": token });
    ...
    let limited = 0;
    for (let i = 0; i < 8; i++) {
      const rr = await post("/auth/login", { email: "ada@example.com", password: "wrong-horse" });
