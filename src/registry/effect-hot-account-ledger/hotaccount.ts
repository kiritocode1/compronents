...
export const makeHotAccount = (options: {
  ...
  Effect.gen(function* () {
      ...
      const takeAll = (i: number): Effect.Effect<number, InsufficientFunds> =>
        i === subs.length
          ? Effect.gen(function* () {
              ...
              if (total < cents) {
                return yield* new InsufficientFunds({ requestedCents: cents, balanceCents: total })
              }
              let remaining = cents
    ...
    return { credit, balance, debit, peakConcurrentHolds: Ref.get(peak) } as const
  })
...
const demo = Effect.gen(function* () {
  ...
  // racing 200 concurrent credits never double-spends or deadlocks.
  {
    ...
    const refusedTyped = refused._tag === "Failure" && String(refused.cause).includes("InsufficientFunds")
