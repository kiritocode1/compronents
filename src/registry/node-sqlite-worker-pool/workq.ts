...
export class JobQueue {
  ...
  enqueue(
    ...
    const r = this.db
      .prepare(
        "INSERT INTO jobs (type, payload, run_at, created_at, max_attempts) VALUES (?, ?, ?, ?, ?)",
      )
      .run(
        type,
  ...
  claim(): Job | undefined {
    const row = this.db
      .prepare(`
			...
			`)
      .get(Date.now()) as
      | (Omit<Job, "payload"> & { payload: string })
      | undefined;
  ...
  fail(
    id: number,
    error: string,
  ): { disposition: "retry" | "dead"; delayMs: number } {
    const row = this.db
    ...
    if (attempts >= row.max_attempts) {
      this.db
        .prepare(
          "UPDATE jobs SET state='dead', attempts=?, finished_at=?, last_error=? WHERE id=?",
        )
        .run(attempts, Date.now(), error, id);
      return { disposition: "dead", delayMs: 0 };
    }
    const delayMs = backoffDelay(row.attempts, this.backoff);
    this.db
      .prepare(
        "UPDATE jobs SET state='queued', attempts=?, run_at=?, last_error=? WHERE id=?",
      )
      .run(attempts, Date.now() + delayMs, error, id);
  ...
  summary(): unknown[] {
    return this.db
      .prepare(
        "SELECT id, type, state, attempts, last_error, result FROM jobs ORDER BY id",
      )
      .all();
  }
...
if (!isMainThread && parentPort) {
  const handlers: Record<
    string,
    (payload: Record<string, unknown>, attempt: number) => Promise<unknown>
  > = {
    async square(p) {
    ...
    async flaky(p, attempt) {
      ...
      if (attempt < succeedOn)
        throw new Error(
          `transient failure, attempt ${attempt} of ${succeedOn}`,
        );
  ...
  parentPort.on("message", async (msg: Dispatch) => {
    try {
      ...
      parentPort!.postMessage({
        jobId: msg.jobId,
        ok: true,
        result: await handler(msg.payload, msg.attempt),
      });
    } catch (err) {
      parentPort!.postMessage({
        jobId: msg.jobId,
      ...
      worker.on(
        ...
        (msg: {
          ...
          if (msg.ok) {
            ...
            console.log(
              styleText("green", `done  #${job.id} ${job.type}`),
              JSON.stringify(msg.result),
            );
          } else {
            const { disposition, delayMs } = this.queue.fail(
              job.id,
              msg.error ?? "unknown",
            );
            if (disposition === "retry") {
              console.log(
                styleText(
                  "yellow",
                  `retry #${job.id} ${job.type} in ${delayMs}ms: ${msg.error}`,
                ),
              );
            } else {
              console.log(
                styleText("red", `dead  #${job.id} ${job.type}: ${msg.error}`),
              );
            }
          }
          this.tick();
...
if ((import.meta as { main?: boolean }).main && isMainThread) {
  const { positionals, values } = parseArgs({
    ...
    options: {
      db: {
        type: "string",
        default: new URL("./workq-demo.db", import.meta.url).pathname,
      },
  ...
  if (cmd === "demo") {
    for (const suffix of ["", "-wal", "-shm"])
      rmSync(values.db + suffix, { force: true });
    const queue = new JobQueue(values.db, {
      backoff: { baseMs: 150, capMs: 1500 },
    });
    ...
    console.log(
      styleText(
        "bold",
        `demo: 7 jobs enqueued (one recovers after 2 failures, one is doomed), pool=${values.workers}`,
      ),
    );
    ...
    console.error(
      "usage: node --experimental-sqlite workq.ts demo | enqueue <type> [json] | run | status  [--db path] [--workers n]",
    );
    process.exitCode = 2;
  }
  ...
  async function runUntilDrained(
    ...
    if (recovered)
      console.log(
        styleText(
          "yellow",
          `recovered ${recovered} orphaned running job(s) from a previous crash`,
        ),
      );
