...
export interface ObjectStore {
  ...
  readonly acknowledged: (uploadId: string) => Effect.Effect<Map<number, string>>
...
export const makeObjectStore = (options: {
  ...
  Effect.gen(function* () {
    ...
    const uploadPart = (uploadId: string, partNumber: number, bytes: string) =>
      Effect.gen(function* () {
        ...
        yield* Ref.update(uploads, (m) => {
          ...
          next.set(uploadId, { ...u, parts: new Map(u.parts).set(partNumber, etag) })
          return next
        })
        return etag
      })
    ...
    const orphanedParts = Ref.get(uploads).pipe(
      Effect.map((m) =>
        [...m.values()].filter((u) => !u.completed).reduce((n, u) => n + u.parts.size, 0),
      ),
    )

    const acknowledged = (uploadId: string) =>
      Ref.get(uploads).pipe(Effect.map((m) => new Map(m.get(uploadId)?.parts ?? [])))

    return { initiate, uploadPart, complete, abort, orphanedParts, acknowledged, objects } as const
  })
...
export const multipartUpload = (
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const uploadId =
        options.resumeFrom ?? (yield* Effect.acquireRelease(store.initiate(key), () => Effect.void))
      // The abort finalizer: runs on failure AND interrupt, skips after complete.
      yield* Effect.addFinalizer(() =>
        Ref.get(done).pipe(Effect.flatMap((ok) => (ok ? Effect.void : store.abort(uploadId)))),
      )
...
const demo = Effect.gen(function* () {
  ...
  const eightParts = Array.from({ length: 8 }, (_, i) => `chunk-${i + 1}`.padEnd(200, "x"))

  // Property 1: a flaky part is retried alone; the object still completes.
  {
    const store = yield* makeObjectStore({ flakyParts: [5] })
    const { uploadedNow } = yield* multipartUpload(store, "backup.tar", eightParts, {
      concurrency: 4,
      retriesPerPart: 2,
    })
  ...
  // Property 2: a crash mid-upload aborts the session; nothing is left billing.
  {
    ...
    const exit = yield* Effect.exit(
      multipartUpload(flakyStore, "video.mp4", eightParts, { concurrency: 2, retriesPerPart: 0 }),
    )
  ...
  // Property 3: resume re-uploads only the missing parts.
  {
    ...
    const { uploadedNow } = yield* multipartUpload(store, "dataset.parquet", eightParts, {
      concurrency: 4,
      retriesPerPart: 1,
      resumeFrom: uploadId,
    })
