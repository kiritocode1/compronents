...
export interface PasswordVault {
  ...
  readonly login: (
    user: string,
    password: string,
  ) => Effect.Effect<void, InvalidCredentials>;
...
export const makePasswordVault = (options?: {
  ...
  Effect.gen(function* () {
    ...
    const register = (
      ...
      Ref.update(records, (m) =>
        new Map(m).set(user, hashPassword(password, atParams ?? params)),
      );
      ...
      Effect.gen(function* () {
        ...
        if (needsRehash(stored, params)) {
          yield* Ref.update(records, (m) =>
            new Map(m).set(user, hashPassword(password, params)),
          );
        }
      });
...
const demo = Effect.gen(function* () {
  ...
  // both still verify.
  {
    ...
    yield* check(
      ...
      a !== undefined &&
        g !== undefined &&
        a !== g &&
        bothLogin.every((e) => e._tag === "Success"),
  ...
  {
    yield* vault.register("torvald", "kernel-panic-42", {
      N: 1024,
      r: 8,
      p: 1,
    });
  ...
  // Property 4: a tampered record never verifies.
  {
    ...
    const tampered =
      record.slice(0, -2) + (record.endsWith("00") ? "01" : "00");
    yield* check(
