...
export const makeHashRing = (options: {
  ...
  Effect.gen(function* () {
    ...
    const addNode = (node: string) =>
      Ref.update(state, (s) =>
        s.members.includes(node) ? s : rebuild([...s.members, node]),
      );
...
const demo = Effect.gen(function* () {
  ...
  // Property 1: adding a 5th node moves ~1/5 of the keys; modulo moves ~4/5.
  {
    ...
    for (const key of KEYS)
      if (fnv1a(key) % 4 !== fnv1a(key) % 5) moduloMoved++;
    ...
    let strayMoves = 0;
    for (const key of KEYS) {
      if (before.get(key) !== after.get(key) && after.get(key) !== "cache-5")
        strayMoves++;
    }
    yield* check(
      "moved keys all land on the new node",
      strayMoves === 0,
      `${strayMoves} keys moved between old nodes`,
    );
  }
  ...
  // Property 2: vnodes even out the load.
  {
    ...
    for (const node of placement.values())
  ...
  // Property 3: removing a node strands only its own keys.
  {
    ...
    const survivors =
      KEYS.length - [...before.values()].filter((n) => n === "cache-3").length;
    yield* check(
