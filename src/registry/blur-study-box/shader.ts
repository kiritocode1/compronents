...
struct VertexOutput {
  ...
  @location(4) @interpolate(flat) proxySize: vec2f,
}
...
fn fragment_main(input: VertexOutput) -> @location(0) vec4f {
  // WGSL only allows derivatives in uniform control flow, so this is taken
  // before any branch even though only the hairline reads it.
  let texel = fwidth(input.localPosition);

  if (input.kind > 0.5) {
    // The hairline is the only hard-edged geometry here, so it antialiases
    // itself from its own quad's derivative instead of making every capsule
    // fragment pay for a multisampled target.
    let edge = input.proxySize - abs(input.localPosition);
    let coverage =
      clamp(edge.x / max(texel.x, 0.000001) + 0.5, 0.0, 1.0) *
      clamp(edge.y / max(texel.y, 0.000001) + 0.5, 0.0, 1.0);
    return vec4f(
      0.42326767,
      0.4677838,
      0.42326767,
      0.72 * params.showFloor * coverage,
    );
  }
  ...
  for (var sample = 0u; sample < 96u; sample += 1u) {
    ...
    let jitteredSample = f32(sample) + 0.5 + noise * params.ditherStrength;
    let sampleZ = halfDepth - stepDepth * jitteredSample;
