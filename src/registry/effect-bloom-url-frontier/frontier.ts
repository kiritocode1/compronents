...
export const makeUrlFrontier = (options: {
  ...
  Effect.gen(function* () {
    ...
    const m = Math.ceil(
      (-options.capacity * Math.log(options.falsePositiveRate)) / (Math.LN2 * Math.LN2),
    )
    ...
    const discover = (url: string) =>
      Effect.gen(function* () {
        ...
        const seen = yield* Ref.modify(gate, (n) => [testAndSet(url), n + 1] as const)
...
const demo = Effect.gen(function* () {
  ...
  const urls = Array.from({ length: N }, (_, i) => `https://site-${i % 500}.example/page/${i}`)
  let enqueued = 0
