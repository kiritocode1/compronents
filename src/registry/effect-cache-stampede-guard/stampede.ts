...
import { Cache, Data, type Duration, Effect, Ref, Semaphore } from "effect";
...
const staleWhileRevalidate = (
  cache: Cache.Cache<string, string, OriginUnavailable>,
  key: string,
) =>
  Effect.gen(function* () {
...
const demo = Effect.gen(function* () {
  ...
  // Property 1: 100 concurrent gets of ONE cold key => exactly 1 origin load.
  {
    ...
    const cache = yield* makeCoalescingCache(origin, {
      capacity: 1000,
      ttl: "10 seconds",
      admission: 8,
    });
    ...
    yield* assert(
      ...
      loads === 1,
      `100 concurrent gets caused ${loads} origin load(s), expected 1`,
    );
  }
  ...
  // => origin never sees more than 8 concurrent loads.
  {
    ...
    yield* assert(
      ...
      peak <= 8,
      `peak concurrent origin loads was ${peak}, ceiling 8`,
    );
    yield* assert(
      "distinct keys each loaded once",
      loads === 200,
      `200 distinct keys caused ${loads} loads, expected 200`,
    );
  }
