...
export const makeFailureDetector = (options: {
  ...
  Effect.gen(function* () {
    ...
    const window = yield* Ref.make<Window>({ lastArrival: start, intervals: [] })

    const heartbeat = Effect.gen(function* () {
      const now = yield* Clock.currentTimeMillis
      yield* Ref.update(window, (w) => ({
        lastArrival: now,
        intervals: [...w.intervals, now - w.lastArrival].slice(-options.windowSize),
      }))
    })
    ...
    const status = phi.pipe(Effect.map((p) => ({ phi: p, alive: p < options.threshold })))
...
const demo = Effect.gen(function* () {
  ...
  // A fixed 40ms timeout would have declared it dead here.
  {
    ...
    yield* check("arrival resets suspicion", after < 0.5, `phi fell to ${after.toFixed(2)} on arrival`)
  }
  ...
  {
    const slow = yield* makeFailureDetector({ threshold: 8, windowSize: 32, bootstrapIntervalMs: 80 })
    for (let i = 0; i < 5; i++) {
