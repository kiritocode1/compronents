...
const hasNetScope: boolean = (() => {
	try {
		return execFileSync(process.execPath, ["--help"], { encoding: "utf8" }).includes("--allow-net");
	} catch {
		return false;
	}
})();

export function runPlugin(pluginPath: string, opts: SandboxOptions): Promise<SandboxOutcome> {
		...
		child.on("close", (exitCode, signal) => {
			...
			for (const line of stdout.split("\n")) {
				if (line.startsWith(RESULT_PREFIX)) {
					try {
						result = structuredClone(JSON.parse(line.slice(RESULT_PREFIX.length)));
						ok = true;
					} catch {
			...
			const denied = /code:\s*'ERR_ACCESS_DENIED'[\s\S]*?permission:\s*'(\w+)'/.exec(stderr);
			resolvePromise({
...
async function demo(baseDir: string): Promise<void> {
	...
	writeFileSync(join(dataDir, "notes.txt"), "durable queues beat fragile cron jobs every single time");
	writeFileSync(join(dataDir, "spec.txt"), "plugins read data, plugins never write, the runner decides");
	for (const [name, src] of Object.entries(DEMO_PLUGINS)) {
		writeFileSync(join(pluginDir, name), src.trimStart());
	}

	console.log(styleText("bold", `sandbox demo: whitelist=${dataDir}, netScope=${hasNetScope ? "enforced" : "unavailable on this Node, ceiling documented"}`));
	for (const name of Object.keys(DEMO_PLUGINS)) {
		const out = await runPlugin(join(pluginDir, name), { readDir: dataDir, timeoutMs: 1500 });
		const verdict = out.ok
			? styleText("green", "ALLOWED")
			: out.timedOut
				? styleText("yellow", "KILLED (timeout)")
				: styleText("red", `BLOCKED${out.deniedPermission ? ` (${out.deniedPermission})` : ""}`);
		...
		else if (!out.timedOut) console.log(styleText("dim", "  " + out.stderrTail.split("\n").slice(0, 3).join("\n  ")));
	}
}
...
if ((import.meta as { main?: boolean }).main) {
	...
	if (cmd === "demo") {
		await demo(join(dirname(new URL(import.meta.url).pathname), "sandbox-demo"));
		...
		console.error("usage: node sandbox.ts demo | node sandbox.ts run <plugin.ts> --read-dir <dir> [--timeout ms] [--input json]");
		process.exitCode = 2;
	}
}
