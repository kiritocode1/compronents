...
export const encodeGeohash = (lat: number, lng: number, precision: number): string => {
  let minLat = -90
  let maxLat = 90
  let minLng = -180
  let maxLng = 180
...
export const coveringCells = (lat: number, lng: number, precision: number): readonly string[] => {
  ...
  for (const dy of [-1, 0, 1]) {
    for (const dx of [-1, 0, 1]) {
      const nLat = Math.max(-90, Math.min(90, (b.minLat + b.maxLat) / 2 + dy * dLat))
...
export const makeProximityIndex = (options: {
  ...
  Effect.gen(function* () {
    ...
    return { add, remove, nearby, lastCandidateCount: Ref.get(lastCandidates) } as const
  })
...
const demo = Effect.gen(function* () {
  ...
  for (let i = 0; i < 2000; i++) {
    yield* index.add({ id: `far-${i}`, name: `Far ${i}`, lat: 34 + (i % 50) * 0.01, lng: -118 + Math.floor(i / 50) * 0.01 })
  }
  ...
  // Property 1: a 1km query returns exactly the walkable spots, sorted.
  {
    ...
    yield* check(
      ...
      names.join(" | ") === "Blue Note Cafe | Mercer Ramen | Union Sq Tacos" &&
        results.every((r, i) => i === 0 || results[i - 1].distanceMeters <= r.distanceMeters),
  ...
  // one, the 9-cell covering finds it.
  {
    ...
    const distance = haversineMeters(south.lat, south.lng, north.lat, north.lng)
    const found = yield* index.nearby(south.lat, south.lng, 300)
    yield* check(
      "neighbor scan beats the cell edge",
      northCell !== userCell && distance < 300 && found.some((r) => r.id === "b1"),
  ...
  // removal takes effect immediately.
  {
    ...
    yield* check(
      ...
      a.slice(0, 4) === b.slice(0, 4) && c.slice(0, 2) !== a.slice(0, 2) && after.every((r) => r.id !== "p1"),
