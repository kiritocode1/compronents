...
export interface Bulkhead {
  ...
  readonly run: <A, E>(effect: Effect.Effect<A, E>) => Effect.Effect<A, E | BulkheadRejected>
...
export const makeBulkhead = (options: {
  ...
  Effect.gen(function* () {
    ...
    const run = <A, E>(effect: Effect.Effect<A, E>): Effect.Effect<A, E | BulkheadRejected> =>
      Effect.gen(function* () {
    ...
    const stats = Effect.gen(function* () {
      return { shed: yield* Ref.get(shed), peakInFlight: yield* Ref.get(peakInFlight) }
    })
...
const demo = Effect.gen(function* () {
  ...
  // bulkheads, the same traffic leaves payments untouched.
  {
    const lastPaymentDone = (
      run: <A, E>(e: Effect.Effect<A, E>) => Effect.Effect<A, E | BulkheadRejected>,
      runSlow: <A, E>(e: Effect.Effect<A, E>) => Effect.Effect<A, E | BulkheadRejected>,
    ) =>
      Effect.gen(function* () {
        const t0 = Date.now()
        ...
        yield* Effect.all(
          [
            ...Array.from({ length: 30 }, () => runSlow(slowCall).pipe(Effect.exit)),
            ...Array.from({ length: 30 }, () =>
              run(fastCall)
                .pipe(Effect.tap(() => Ref.update(doneAt, (t) => Math.max(t, Date.now() - t0))))
                .pipe(Effect.exit),
            ),
          ],
    ...
    const shared = yield* makeBulkhead({ name: "shared-pool", permits: 8, waitingRoom: 60 })
    const sharedMs = yield* lastPaymentDone(shared.run, shared.run)

    // isolated: each dependency owns its compartment
    const recs = yield* makeBulkhead({ name: "recommendations", permits: 4, waitingRoom: 30 })
    const payments = yield* makeBulkhead({ name: "payments", permits: 4, waitingRoom: 30 })
  ...
  {
    const recs = yield* makeBulkhead({ name: "recommendations", permits: 4, waitingRoom: 4 })
    yield* Effect.all(
      Array.from({ length: 40 }, () => recs.run(Effect.sleep("20 millis")).pipe(Effect.exit)),
      { concurrency: "unbounded" },
    )
    const { peakInFlight } = yield* recs.stats
    yield* check("permit ceiling holds", peakInFlight <= 4, `40 concurrent callers, peak in-flight ${peakInFlight} (ceiling 4)`)
  }
