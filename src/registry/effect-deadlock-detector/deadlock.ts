...
export interface LockManager {
  ...
  readonly withTransaction: <A, E>(
    tx: string,
    body: (acquire: (resource: string) => Effect.Effect<void, DeadlockVictim>) => Effect.Effect<A, E>,
...
export const makeLockManager = (): Effect.Effect<LockManager> =>
  Effect.gen(function* () {
    const state = yield* Ref.make<LockState>({ owners: new Map(), waiting: new Map() })
    ...
    const findCycle = (s: LockState, target: string, wanted: string): readonly string[] | null => {
    ...
    const withTransaction = <A, E>(
        ...
        Effect.gen(function* () {
          while (true) {
            type Decision =
              ...
              | { readonly _tag: "Wait" }
              | { readonly _tag: "Deadlock"; readonly cycle: readonly string[] }
            const decision = yield* Ref.modify(state, (s): readonly [Decision, LockState] => {
              const owner = s.owners.get(resource)
              ...
              if (cycle !== null) {
                ...
                const waiting = new Map(s.waiting)
                waiting.delete(tx)
                return [{ _tag: "Deadlock", cycle }, { owners: s.owners, waiting }]
              }
            ...
            if (decision._tag === "Deadlock") {
              return yield* new DeadlockVictim({ tx, wanted: resource, cycle: decision.cycle })
            }
      ...
      const releaseAll = Ref.update(state, (s) => {
        const owners = new Map([...s.owners].filter(([, owner]) => owner !== tx))
        const waiting = new Map(s.waiting)
        waiting.delete(tx)
...
const demo = Effect.gen(function* () {
  ...
  // one typed victim; the survivor completes; nothing hangs.
  {
    ...
    const [e1, e2] = yield* Effect.all(
      [Effect.exit(tx("T1", "accounts:alice", "accounts:bob")), Effect.exit(tx("T2", "accounts:bob", "accounts:alice"))],
