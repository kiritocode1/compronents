...
export const makeBroker = (): Effect.Effect<Broker> =>
  Effect.gen(function* () {
    ...
    const commit = (offset: number) => Ref.update(committed, (c) => Math.max(c, offset))
    return { append, poll, commit, committedOffset: Ref.get(committed) } as const
  })
...
): Effect.Effect<{ applied: number; skippedDuplicates: number }, CrashMidProcess> =>
  Effect.gen(function* () {
    let applied = 0
    let skipped = 0
...
const demo = Effect.gen(function* () {
  ...
  // Property 1: crash after apply, before commit; redelivery does not double-apply.
  {
    ...
    const firstRun = yield* Effect.exit(runConsumer(broker, ledger, { crashBeforeCommitAt: 1 }))
    ...
    yield* check("offsets caught up", finalOffset === 2, `committed offset ${finalOffset}`)
  }
