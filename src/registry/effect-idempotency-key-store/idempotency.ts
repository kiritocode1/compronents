...
type Slot<A, E> =
  ...
  | { readonly _tag: "Completed"; readonly value: A; readonly expiresAt: number }
...
export const makeIdempotencyStore = (options: {
  ...
  Effect.gen(function* () {
    ...
    const execute = <A, E>(key: string, effect: Effect.Effect<A, E>): Effect.Effect<A, E> =>
      Effect.gen(function* () {
        ...
        const claim = yield* Ref.modify(slots, (map): readonly [Claim, SlotMap] => {
          ...
          next.set(key, { _tag: "InFlight", done: fresh as Deferred.Deferred<unknown, unknown> })
...
class CardDeclined extends Data.TaggedError("CardDeclined")<{ readonly key: string }> {}
...
const demo = Effect.gen(function* () {
  ...
  // Property 1: 25 concurrent submissions of ONE key => one charge, one receipt.
  {
    const results = yield* Effect.all(
      Array.from({ length: 25 }, () => store.execute("order-771", charge("order-771", 4999))),
  ...
  // Property 4: distinct keys execute independently.
  {
    ...
    yield* assert("distinct keys are independent", after === before + 1, `executions ${before} -> ${after}`)
  }
