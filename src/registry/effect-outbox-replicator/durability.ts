...
class SinkUnavailable extends Data.TaggedError("SinkUnavailable")<{ readonly seq: number }> {}
...
const makeSourceDb = () =>
  Effect.map(
    Ref.make<SourceState>({ records: new Map<string, string>(), outbox: [], nextSeq: 1 }),
...
const applyIdempotent = (replica: Replica, entry: OutboxEntry) =>
  Effect.gen(function* () {
    ...
    yield* Ref.update(replica.records, (m) => new Map(m).set(entry.id, entry.value))
...
const liveWorkers = Metric.gauge("live_background_workers", { description: "background fibers currently alive" })
...
const canaryWorker = (live: Ref.Ref<number>) =>
  Ref.updateAndGet(live, (n) => n + 1).pipe(
    ...
    Effect.ensuring(
      Ref.updateAndGet(live, (n) => n - 1).pipe(Effect.flatMap((n) => Metric.update(liveWorkers, n))),
    ),
  )
...
const demo = Effect.gen(function* () {
  ...
  // record exactly once despite at-least-once re-delivery.
  {
    ...
    for (let i = 1; i <= 6; i++) yield* writeWithOutbox(db, `acct-${i}`, `balance-${i}`)

    // First drain crashes after applying seq 3 but before acking it.
    const firstRun = yield* drain(db, replica, cursor, { crashAfterSeq: 3 }).pipe(Effect.exit)
    ...
    const lostWrites = [...source.records].filter(([id, v]) => replicated.get(id) !== v)

    yield* assert(
      "outbox survives a mid-stream crash with zero loss",
      Exit.isFailure(firstRun) && cursorAfterCrash === 2 && lostWrites.length === 0 && replicated.size === 6,
  ...
  // way the canary catches.
  {
    ...
    yield* Effect.scoped(
      Effect.gen(function* () {
        ...
        for (let i = 0; i < 5; i++) yield* FiberSet.run(set, canaryWorker(live))
        yield* Effect.sleep("30 millis")
        const alive = yield* Ref.get(live)
        yield* assert("FiberSet holds live workers", alive === 5, `${alive} workers alive inside the scope`)
      }),
    )
